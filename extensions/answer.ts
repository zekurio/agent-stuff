/**
 * Vendored from mitsuhiko/agent-stuff (Apache-2.0), extensions/answer.ts.
 * Kept byte-identical below this header so upstream diffs stay readable.
 */
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			if (ctx.mode !== "tui") {
				if (ctx.hasUI) {
					ctx.ui.notify("answer requires interactive terminal mode", "error");
				}
				return;
			}
